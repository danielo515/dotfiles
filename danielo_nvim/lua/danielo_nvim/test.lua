...
__kickstart__LspConfig_LspConfig_Fields_.doY = function() 
  local _ = ___TableWrapper_TableWrapper_Impl_.check(_hx_o({__fields__={doX=true,test=true,nestArr=true,nest=true},doX=99,test=true,nestArr=_hx_o({__fields__={x=true},x=_hx_tab_array({[0]=_hx_o({__fields__={y=true},y="inside array"}), _hx_o({__fields__={y=true},y="second item"})}, 2)}),nest=_hx_o({__fields__={a=true},a=_hx_o({__fields__={renest=true,b=true},renest=99,b=_hx_o({__fields__={c=true},c=_hx_o({__fields__={meganest=true},meganest=88})})})})}));
  __kickstart__LspConfig_LspConfig_Fields_.testMethod(({doX = 99, nest = ({a = ({renest = 99, b = ({c = ({meganest = 88})})})}), nestArr = ({x = ({({y = "inside array"}),({y = "second item"})})}), test = true}));
end
