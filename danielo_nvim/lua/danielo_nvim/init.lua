...
__parser_State.Finished = function(tokens)
	local _x = _hx_tab_array({ [0] = "Finished", 1, tokens, __enum__ = __parser_State }, 3)
	return _x
end
__parser_State.Running = function(buffer, position)
	local _x = _hx_tab_array({ [0] = "Running", 2, buffer, position, __enum__ = __parser_State }, 4)
	return _x
end
...
__parser_Parser.prototype.seek = function(self)
	...
	elseif nextChar1 == 2 then
		local _g1 = _g[2]
		local pos = _g[3]
		nextChar = _G.string.sub(self.string, pos + 1, pos + 1)
	end
	do
		return nextChar
	end
...
__parser_Parser.prototype.tokenize = function(self)
	...
	if isValidChar then
		...
		elseif tmp1 == 2 then
			local buffer = _g[2]
			local pos = _g[3]
			tmp = __parser_State.Running(Std.string(buffer) .. Std.string(nextChar), pos)
		end
	else
		local tmp1 = _g[1]
		if tmp1 == 0 then
		...
		elseif tmp1 == 2 then
			...
			local pos = _g[3]
			self.tokens:push(buffer)
			tmp = __parser_State.Running("", pos)
		end
	end
	self.state = tmp
	do
...
__parser_ClassName.main = function()
	...
	__haxe_Log.trace(
		...
		_hx_o({
			...
			fileName = "src/parser/ClassName.hx",
			lineNumber = 88,
...
__plenary__Job_JobOpts_Impl_._new = function(command, args)
	...
	local this1 = {
		...
		description = (function()
			...
			if description ~= nil then
				...
				_hx_1 = Std.string(
					Std.string(Std.string(Std.string("") .. Std.string(groupName)) .. Std.string(":["))
						.. Std.string(pattern)
				) .. Std.string("]")
			end
			return _hx_1
...
__vim_DanieloVim.chezmoi = function(args)
	...
	local this1 = { command = "chezmoi", arguments = ret, on_exit = vim.pretty_print }
	local job = __plenary_Job.new(__plenary_Job, this1)
	do
		return job:start()
	end
end
__vim_DanieloVim.main = function()
	__haxe_Log.trace(
		__vim_DanieloVim.chezmoi(_hx_tab_array({ [0] = "-v" }, 1)),
		_hx_o({
			__fields__ = { fileName = true, lineNumber = true, className = true, methodName = true },
			fileName = "src/vim/Vim.hx",
			lineNumber = 49,
			className = "vim.DanieloVim",
			methodName = "main",
		})
	)
end
