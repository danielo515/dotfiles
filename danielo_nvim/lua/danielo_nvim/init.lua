...
__vim_DanieloVim.chezmoi = function(args) 
  ...
  local args = this1;
  local job = require"plenary.job":new(args);
  do return job:start() end;
end
__vim_DanieloVim.main = function() 
  __haxe_Log.trace(__vim_DanieloVim.chezmoi(_hx_tab_array({[0]="-v"}, 1)), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/vim/Vim.hx",lineNumber=49,className="vim.DanieloVim",methodName="main"}));
end
