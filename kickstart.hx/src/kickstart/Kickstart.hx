...
function keymaps() {
	...
	Keymap.set(t([Normal, Visual]), '<Space>', '<Nop>', {desc: 'do nothing', silent: true, expr: false});
	Keymap.set(t([Normal]), 'k', "v:count == 0 ? 'gk' : 'k'", {desc: 'up when word-wrap', silent: true, expr: true});
	Keymap.set(t([Normal]), 'j', "v:count == 0 ? 'gj' : 'j'", {desc: 'down when word-wrap', silent: true, expr: true});
}
...
function onAttach(x:Dynamic, bufnr:Buffer) {
	...
	nmap('<leader>wl', () -> Vim.print(LspBuf.list_workspace_folders()), '[W]orkspace [L]ist Folders'); // Create a command `:Format` local to the LSP buffer
 ...
 */
function main() {
	Vimx.autocmd("Kickstart", t([BufWritePost]), Fn.expand(MYVIMRC), "Reload the config", () -> Vim.cmd("source <afile> | PackerCompile"));
	Vimx.autocmd("Kickstart-yank", [TextYankPost], "*", "Highlight on yank", kickstart.Untyped.higlightOnYank);
