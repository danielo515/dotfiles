...
__plugins__Packer_Packer_Fields_ = _hx_e()
...
__kickstart__Pure_Pure_Fields_.main = function()
	...
	local plugins = { spec.name, cmd = spec.cmd, event = spec.event, config = spec.config }
	local spec = _hx_o({ __fields__ = { name = true }, name = "kylechui/nvim-surround" })
	local plugins =
		_hx_tab_array({ [0] = plugins, { spec.name, cmd = spec.cmd, event = spec.event, config = spec.config } }, 2)
...
__plugins__Packer_Packer_Fields_.ensureInstalled = function()
