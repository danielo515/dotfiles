...
___Main_Main_Fields_.main = function() 
  vim.api.nvim_create_user_command("HaxeCmd", function(args) 
    vim.print(args);
    vim.print(vim.spell.check("Hello bru! Hau are you?")[1][1]);
  ...
  local tmp = __vim_FunctionOrString.Cb(function() 
    vim.print("Hello from axe", vim.fn.expand(_G.string.format("%s%s", "%", ":p")));
    do return true end;
  end);
...
__kickstart__Kickstart_Kickstart_Fields_.main = function() 
  ...
  local spec = _hx_o({__fields__={name=true,requires=true},name="kdheepak/lazygit.nvim",requires=({"nvim-lua/plenary.nvim"})});
  local plugins3 = { 
        spec.name, 
        disable=spec.disable,
        as=spec.as,
  ...
  if (is_bootstrap) then 
    vim.print("==================================");
    vim.print("    Plugins are being installed");
    vim.print("    Wait until Packer completes,");
    vim.print("       then restart nvim");
    vim.print("==================================");
    do return end;
  end;
...
__kickstart__Kickstart_Kickstart_Fields_.setupPlugins = function() 
  ...
  local _v_ = vimx.safeRequire("kylechui/nvim-surround");
  if (_v_ ~= nil) then 
    _v_.setup();
  end;
    ...
    local mapping = cmp1.insert(
{
    ...
    cmp.setup.cmdline("/", ({mapping = mapping, snippet = nil, sources = ({({name = "buffer"}),({name = "cmdline_history"})})}));
  end;
  ...
  if (lspconfig ~= nil) then 
    ...
    if (capabilities ~= nil) then 
      ...
      capabilities = config_capabilities,
          ...
          vim.print(Std.string("Ignoring ") .. Std.string(server_name)); end;
        end}));
      end;
    end;
  end;
